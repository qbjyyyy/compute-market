...
| 模块   | 做什么                | 不做什么           |
| ---- | ------------------ | -------------- |
| 智能合约 | 管规则、管钱、发事件         | 不跑计算、不调用 iExec |
| 后端   | 监听事件、调度 iExec、同步状态 | 不碰用户私钥         |
| 前端   | 钱包交互、发交易、展示状态      | 不存资产、不跑计算      |
...
# 智能合约模块contracts
智能合约不直接执行计算，也不直接创建 iExec 任务，而是定义算力交易规则 + 托管资金 + 触发链下计算流程。
...
合约并不关心“计算怎么跑”，只记录：任务 ID 发起用户 支付金额 当前状态
